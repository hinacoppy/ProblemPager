//examhistory_class.js
//called from examhistory.html
"use strict";

class ExamHistory {
  constructor() {
    this.setEventListener(); //イベントリスナー登録
    this.showHistoryTable(); //テーブル表示
  }
  ...
  actionDeleteButton() {
    ...
    if (dirty) { //checkが入っていてデータ編集が必要なら
      ...
      this.setDataToStorage(exampager); //ローカルストレージを更新して
      location.reload(); //ページを再表示(テーブルを最新データで再表示)
    }
  }

  getGridJsSettingObject() {
    const settings = { //GridJS config
      data: this.getDataFromStorage(), //表示データ
      ...
      columns: [
        ...
        { name: "名称",       formatter: (_, row) => this.getTitle(row) },
        { name: "回答数",     id: "response" },
        { name: "正答数",     id: "correct" },
        { name: "エラー合計", id: "errorsum" },
        { name: "削除",       formatter: (_, row) => gridjs.html(this.makeCheckbox(row)), sort: false },
      ],
      style: {
        th: {
          "text-align": "center",
  ...
  setEventListener() {
    ...
    deletebtn.addEventListener("click", () => { this.actionDeleteButton(); });
  ...
  showHistoryTable() {
    const table = new gridjs.Grid(this.getGridJsSettingObject());
